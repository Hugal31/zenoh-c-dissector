...
int dissect_zint(tvbuff_t *tvb, proto_tree *tree, int offset, int hfindex, proto_item **item, uint64_t *value)
{
    ...
    const uint64_t ret = read_zint(tvb, &offset);
    const int length = offset - start;
    const header_field_info *info = proto_registrar_get_nth(hfindex);
    proto_item *tree_item;
    if (ret < (1UL << 32) && FT_IS_UINT32(info ? info->type : FT_UINT64))
        tree_item = proto_tree_add_uint(tree, hfindex, tvb, start, length, ret);
    else
        tree_item = proto_tree_add_uint64(tree, hfindex, tvb, start, length, ret);
...
{
    DISSECTOR_ASSERT(length <= MAX_ZID_SIZE);
    ...
    const int last_char_offset = offset + length - 1;
    for (int i = 0; i < length; ++i)
        snprintf(zid_buff + (2 * i), 3, "%02x", (unsigned)tvb_get_uint8(tvb, last_char_offset - i));
