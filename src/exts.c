...
static int dissect_ext(tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree, int offset, ext_dissector_table_t dissector_table, void *data, bool *has_more) {
...
void dissect_qos_type(tvbuff_t * tvb, packet_info * pinfo _U_, proto_tree * tree, int offset, int length, void *data _U_) {
  ...
  proto_item_append_text(item, ", congestion: %s, express: %s }", blocking ? "Block" : "Drop", express ? "True" : "False");
}
